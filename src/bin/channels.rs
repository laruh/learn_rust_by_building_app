...
// * Use `cargo test --bin channels` to test your program to ensure all cases are covered
...
fn main() {
    ...
    s.send(LightMsg::On).expect("panic");
    s.send(LightMsg::ChangeColor(255, 0, 0)).expect("panic");
    s.send(LightMsg::ChangeColor(0, 128, 0)).expect("panic");
    s.send(LightMsg::ChangeColor(0, 0, 255)).expect("panic");
    s.send(LightMsg::Off).expect("panic");
    s.send(LightMsg::Disconnect).expect("panic");

    light.join().expect("failed");
}
